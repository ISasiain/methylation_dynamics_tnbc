...
orig_epi <- read.table("/Users/in2245sa//PhD/Projects/immune_spatial/ecosystem_analysis/data/nmfClusters_groupVariablesWithAnno_3groupBasal_2groupNonBasal_byAtac_bySd.txt", sep = "\t")
...
#       
#       # Saving network
#       my_filename <- paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_distal/", "n_", sample_n, "_beta_", beta, "_resampling_", run, ".rds" )
...
cor <- WGCNA::cor
 ...
 for (sample_n in samples_n) {
   ...
   for (run in runs) {
     ...
     eps <- 0.01
     sampled_betas <- pmin(pmax(sampled_betas, eps), 1 - eps)
     
     # Transform to M-values
     sampled_betas <- log2(sampled_betas / (1 - sampled_betas))
     ...
     for (beta in betas) {
       ...
       # Saving network
       my_filename <- paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_distal/", "filtered_before_resampling_n_", sample_n, "_beta_", beta, "_resampling_", run, ".rds" )
       saveRDS(netwk, file = my_filename)
...
#       
#       # Saving network
#       my_filename <- paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_promoter/", "n_", sample_n, "_beta_", beta, "_resampling_", run, ".rds" )
#       saveRDS(netwk, file = my_filename)
#       
#     }
#   }
...
for (sample_n in samples_n) {
  ...
  for (run in runs) {
    ...
    for (beta in betas) {
      ...
      # Saving network
      my_filename <- paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_promoter/", "filtered_before_resampling_n_", sample_n, "_beta_", beta, "_resampling_", run, ".rds" )
      saveRDS(netwk, file = my_filename)
...
#       
#       # Saving network
#       my_filename <- paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/", "n_", sample_n, "_beta_", beta, "_resampling_", run, ".rds" )
#       saveRDS(netwk, file = my_filename)
#       
#     }
#   }
  ...
  for (run in runs) {
    ...
    for (beta in betas) {
      ...
      netwk <- blockwiseModules(sampled_betas,               
                                ...
                                verbose = 3)
      # Saving network
      my_filename <- paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/", "filtered_before_resampling_n_", sample_n, "_beta_", beta, "_resampling_", run, ".rds" )
      saveRDS(netwk, file = my_filename)
...
my_files <- list.files("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/")
my_files_100 <- my_files[grepl("^n_100_beta_7_*", my_files)]

my_files_100_filbefore <- my_files[grepl("^filtered_before_resampling_n_100_beta_7_*", my_files)]
...
for (file in my_files_100_filbefore) {
  ...
  # Appending cassettes
  list_of_cassettes[[file]] <- readRDS(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/",
                                              file))$colors
  # Appending samples
  list_of_samples[[file]] <- rownames(readRDS(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/",
                                                       file))$MEs)
}
...
# col_annotation <- HeatmapAnnotation(df=data.frame("Epitype" = orig_epi[list_of_samples[[file_names]], "NMF_atacproximal"]),
...
original_assigments_df <- data.frame("CpG" = names(proximal_7$colors),
                            "Cassettes" = proximal_7$colors)
...
for (cassette_to_analyse in 1:5) {
  ...
  for (resampling_file in unique(resampling_order$Resampling)) {
    my_cassettes <- readRDS(paste0(
      "/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/",
      resampling_file
    ))$colors
...
#     
#     a <- readRDS(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/cassettes_resampling/all_proximal/", file))
#     
