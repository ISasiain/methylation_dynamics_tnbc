...
for (current_gene_id in gene_ids) {
  ...
  cpgs_in_cassette <- names(promoter_7$colors)[promoter_7$colors == 10]
  ...
  pvals_gex_methylation[current_gene_id] <- wilcox.test(as.numeric(gex.data[current_gene_id,]) ~ promoter_state)$p.value
  
}

# p adjustment
print(p.adjust(pvals_gex_methylation, method = "BH"))
...
chi_pvals <- sapply(1:5, function(idx) {
  chisq.test(clusters_methylation[idx, ], pam50_annotation)$p.value
})

chi_pvals_adj <- p.adjust(chi_pvals, method = "BH")

pval_stars_adj <- sapply(chi_pvals_adj, pval_to_stars)


chi_p_annotation <- rowAnnotation(
  pvalue = anno_text(pval_stars_adj, gp = gpar(fontsize = 10))
)
...
#
# ANALYSING CASSETTES OF INTEREST. 9
#

# Defining cpgs of interest
cpgs_of_interest <- names(promoter_7$colors)[promoter_7$colors == 9]
...
# Fredlund immune response vs cluster
