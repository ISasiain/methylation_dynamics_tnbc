...
summary_prom7 <- read.csv("/Users/in2245sa/PhD/Projects/project_3/analysis/promoter_cassettes/summary_of_cassettes/summary_beta_7.csv")
rownames(summary_prom7) <- as.character(summary_prom7$Cassette)
summary_prom7$Cassette <- NULL

# DISTAL
distal_7 <- readRDS("/Volumes/Data/Project_3/detected_cassettes/distal/cassettes_beta_7.rds")

summary_dis7 <- read.csv("/Users/in2245sa/PhD/Projects/project_3/analysis/distal_cassettes/summary_of_cassettes/summary_beta_7.csv")
rownames(summary_dis7) <- as.character(summary_dis7$Cassette)
summary_dis7$Cassette <- NULL

# PROXIMAL
proximal_7 <- readRDS("/Volumes/Data/Project_3/detected_cassettes/proximal/cassettes_beta_7.rds")

summary_prox7 <- read.csv("/Users/in2245sa/PhD/Projects/project_3/analysis/proximal_cassettes/summary_cassettes/summary_beta_7.csv")
rownames(summary_prox7) <- as.character(summary_prox7$Cassette)
summary_prox7$Cassette <- NULL
...
for (cassette in c(1,2,3,4,5,6,7)) {
  ...
  betas <- t(betaAdj[names(promoter_7$colors)[promoter_7$colors == cassette], x$PD_ID])
  ...
  # Extract adjusted cpgs per cassette
  betas <- t(betaAdj[names(proximal_7$colors)[proximal_7$colors == cassette], x$PD_ID])
  ...
  # Extract adjusted cpgs per cassette
  betas <- t(betaAdj[names(distal_7$colors)[distal_7$colors == cassette], x$PD_ID])
...
long_df[long_df$ARI > 0.55,]

#
# CLUSTERING SAMPLES. Basal-Non Basal
#
...
my_cpgs_prom <-  c(
  names(promoter_7$colors)[promoter_7$colors == "1"]
)

my_cpgs_dis <-  c(
  names(distal_7$colors)[distal_7$colors == "3"]
)

my_cpgs_prox <- c(
  names(proximal_7$colors)[proximal_7$colors == "1"],
  names(proximal_7$colors)[proximal_7$colors == "2"],
  names(proximal_7$colors)[proximal_7$colors == "4"]
)
...
groupings_df <- data.frame(matrix(nrow = length(colnames(summary_prox7)), ncol = 5))
rownames(groupings_df) <- colnames(summary_prox7)
...
cpgs_in_cassette <- c(names(promoter_7$colors[promoter_7$colors == 1]))
...
# Identifying genes in cassette 1
cpgs_in_cassette <- names(proximal_7$colors[proximal_7$colors %in% c(1)])
genes_in_cassette <- unique(genes[cpgs_in_cassette])
...
left_annotation <- rowAnnotation("CpG_in_cassette" = names(genes)[genes == current_gene_id] %in% names(promoter_7$colors)[promoter_7$colors == 1],
...
# CpG annotation. Context and included in cassette
left_annotation <- rowAnnotation("CpG_in_cassette" = names(genes)[genes == current_gene_id] %in% names(proximal_7$colors)[proximal_7$colors == 1],
                                 "Context"= annoObj$CpG_context[annoObj$illuminaID %in% names(genes)[genes == current_gene_id]],
                                 col=list("Context"=c("Distal" = "#f8766d", 
                                                      "Promoter" = "#00ba38", 
                                                      "Proximal" = "#619cff"),
