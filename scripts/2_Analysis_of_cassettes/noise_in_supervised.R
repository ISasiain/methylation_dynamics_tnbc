...
# Function to group small cassettes into "Others"
...
compare_groups <- function(g1, g2, context = "Distal") {
  ...
  # Get cassette counts
  if (context == "Promoter") {
    tbl <- table(promoter_7$colors[ctx_cpgs])
  } else if (context == "Proximal") {
    tbl <- table(proximal_7$colors[ctx_cpgs])
  } else {
    tbl <- table(distal_7$colors[ctx_cpgs])
  }
