...
summarize_cassettes <- function(files, output_dir) {
  for (file in files) {
    ...
    for (cassette in unique(cpgs_to_cassette)) {
      ...
      eps <- 0.01
      m_values <- pmin(pmax(betas, eps), 1 - eps)
      m_values <- log2(m_values / (1 - m_values))
      
      # Perform PCA on transposed data (samples as rows)
      pca <- prcomp(t(m_values), center = TRUE, scale. = TRUE)
      pc1 <- pca$x[, 1]
      
      # Calculate mean methylation per sample
      mean_methylation <- colMeans(m_values, na.rm = TRUE)
...
# List to plot
all_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/all/", full.names = TRUE, pattern = "*.rds")
...
summary_df$beta <- factor(summary_df$beta, levels = c(5, 6, 7, 8, 10, 15))

# Create the plot
ggplot(summary_df, aes(x = beta)) +
  geom_bar(aes(y = num_cassettes), stat = "identity", fill = "blue", alpha = 0.6) +
  geom_point(aes(y = mean_cassette_length ), color = "red", size = 3) +  # Adjust scaling factor as needed
  geom_line(aes(y = mean_cassette_length , group = 1), color = "red", size = 1) +  # Adjust scaling factor as needed
  labs(x = "Beta",
       y = "Number of Cassettes") +
  scale_y_continuous( sec.axis = sec_axis(~ ., name = "Mean Cassette Length")) +  # Adjust scaling factor as needed
  theme_bw()
...
cassette_df$beta <- factor(cassette_df$beta, levels = c(5, 6, 7, 8, 10, 15))
...
for (file in all_files) {
  ...
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/all_cassettes/diff_betas/heatmap_beta_", beta, ".png"), width = 4000, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}


# UNADJASTED FROM UNADJUSTED CASSETTES
...
# Create the plot
 a <- ggplot(summary_df, aes(x = beta)) +
  geom_bar(aes(y = num_cassettes), stat = "identity", fill = "blue", alpha = 0.6) +
  geom_point(aes(y = mean_cassette_length), color = "red", size = 3) +  # Adjust scaling factor as needed
  geom_line(aes(y = mean_cassette_length, group = 1), color = "red", size = 1) +  # Adjust scaling factor as needed
  labs(x = "Beta",
...
cassette_df <- cassette_df %>%
  group_by(beta) %>%
  mutate(cassette = ifelse(proportion < 0.05, "Other", as.character(cassette))) %>%
  group_by(beta, cassette) %>%
  summarise(proportion = sum(proportion), .groups = "drop")
...
# Plot
b <- ggplot(cassette_df, aes(x = beta, y = proportion, fill = cassette)) +
  geom_bar(stat = "identity", position = "fill") +
  scale_y_continuous(labels = scales::percent_format()) +
  labs(x = "Beta",
       y = "Proportion",
...
for (file in all_files) {
  ...
  # Save the heatmap to a file
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/all_cassettes/diff_betas/heatmap_beta_", beta, "_unadjusted_from_unadjusted.png"), width = 4000, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
# List all files
distal_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/distal/", full.names = TRUE, pattern = "*.rds")
...
for (file in distal_files) {
  ...
  # Save the heatmap to a file 
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/distal_cassettes/diff_betas/heatmap_beta_", beta, ".png"), width = 4000, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
ggplot(summary_df, aes(x = beta)) +
  ...
  geom_point(aes(y = mean_cassette_length ), color = "red", size = 3) +  # Adjust scaling factor as needed
  geom_line(aes(y = mean_cassette_length , group = 1), color = "red", size = 1) +  # Adjust scaling factor as needed
  labs(title = "Distal cassettes (Var > 0.05)",
       x = "Beta",
       y = "Number of Cassettes") +
  scale_y_continuous(sec.axis = sec_axis(~ . , name = "Mean Cassette Length")) +  # Adjust scaling factor as needed
  theme_classic()
...
for (file in distal_files) {
  ...
  heatmap <- Heatmap(beta_subset, 
                     ...
                     show_column_names = FALSE, 
                     row_split = cassette_factor, 
                     row_title = unique(cassette_factor),  
                     column_title = paste("CpG Methylation Heatmap by Cassettes (Beta =", beta, ")"),
                     top_annotation = column_annotation_short,
                     #left_annotation = row_annotation,
                     #right_annotation = atac_annotation,
  ...
  # Save the heatmap to a file 
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/distal_cassettes/diff_betas/heatmap_beta_", beta, "_only_atac.png"), width = 4000, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
for (file in distal_files) {
  ...
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/distal_cassettes/diff_betas/heatmap_beta_", beta, "_VALIDATION.png"), width = 5500, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
for (file in distal_files) {
  ...
  # Save the heatmap to a file with double size
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/distal_cassettes/diff_betas/heatmap_beta_", beta, "_only_atac_VALIDATION.png"), width = 5500, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
# List all files
promoter_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/promoter/", full.names = TRUE, pattern = ".rds")
...
for (file in promoter_files) {
  ...
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/promoter_cassettes/diff_betas/heatmap_beta_", beta, ".png"), width = 4000, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
for (file in promoter_files) {
  ...
  # Save the heatmap to a file with double size
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/promoter_cassettes/diff_betas/heatmap_beta_", beta, "_VALIDATION.png"), width = 5500, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
# List all files
proximal_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/proximal/", full.names = TRUE, pattern = "*.rds")
...
for (file in proximal_files) {
  ...
  # Save the heatmap to a file
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/proximal_cassettes/diff_betas/heatmap_beta_", beta, ".png"), width = 4000, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
for (file in proximal_files) {
  ...
  # Save the heatmap to a file with double size
  png(paste0("/Users/in2245sa/PhD/Projects/project_3/analysis/proximal_cassettes/diff_betas/heatmap_beta_", beta, "_VALIDATION.png"), width = 5500, height = 2800, res = 600, type = "cairo")
  draw(heatmap)
  dev.off()
}
...
summarize_cassettes(distal_files, "/Users/in2245sa/PhD/Projects/project_3/analysis/distal_cassettes/summary_of_cassettes")

# Promoter cassettes
promoter_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/promoter/", full.names = TRUE, pattern = "*.rds")

summarize_cassettes(promoter_files, "/Users/in2245sa/PhD/Projects/project_3/analysis/promoter_cassettes/summary_of_cassettes")

# Proximal cassettes
proximal_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/proximal/", full.names = TRUE, pattern = "*.rds")

summarize_cassettes(proximal_files, "/Users/in2245sa/PhD/Projects/project_3/analysis/proximal_cassettes/summary_cassettes")
