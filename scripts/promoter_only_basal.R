...
# Getting basal group
promoter_betas <- promoter_betas[,rownames(groupings_df)[groupings_df==1]]
...
for (beta in betas) {
  ...
  # Saving network
  my_filename <- paste0("/Volumes/Data/Project_3/detected_cassettes/promoter/only_basal_cassettes_beta_", beta, ".rds" )
...
# List all files
promoter_files <- list.files("/Volumes/Data/Project_3/detected_cassettes/promoter/", full.names = TRUE, pattern = "*only_basal*")
...
for (file in promoter_files) {
  ...
  pdf(paste0("/Users/isasiain/PhD/Projects/project_3/analysis/promoter_cassettes/diff_betas/only_basal_heatmap_beta_", beta, ".pdf"), width = 14, height = 10)  # Adjust width and height as needed
  draw(heatmap)
  dev.off()
}
